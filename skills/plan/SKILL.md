...
2. Gather candidates:
   ...
   - `findExtracts` (query, `isWishlisted: true`, relevant `extractTypes`/`tagNames`) for wishlist items that fit.
   - `findCollections` if the user names a collection (e.g. "my Tokyo list").
...
4. Offer to save the finished plan back into Albo with `saveImport` and `action: "createMarkdown"` so it shows up in the user's app.
5. If a collection for the trip exists, offer to add any newly relevant items with `changeCollection`.
...
- Use `readCollectionMemory` for existing trip constraints. Use `changeCollectionMemory` only when the user states a durable collection-specific preference worth retaining (e.g. "vegetarian", "no museums").
