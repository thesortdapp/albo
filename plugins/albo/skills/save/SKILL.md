...
1. Work out what the user wants to save:
   - **A URL** (TikTok, Instagram, YouTube, article, recipe page, …) → use `manageImports` with `action: "uploadUrl"` so Albo's pipeline extracts the content automatically.
   - **A described item without a URL** (e.g. "remember this restaurant: Padella in London") → use `manageImports` with `action: "createMarkdown"` to capture the details, or check `getAvailableFilters` for supported extract types first.
